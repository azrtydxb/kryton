 ...
 * Validates that userId looks like a safe identifier (defense in depth).
 */
...
export async function provisionUserNotes(
  ...
  for (const [relativePath, content] of Object.entries(SAMPLE_NOTES)) {
    ...
    if (indexNote) await indexNote(relativePath, content, userId);
    if (updateGraph) await updateGraph(relativePath, content, userId);
  }
}

/**
 * Move any non-safe-id entries in the notes base directory to a backup folder.
 */

---
title: CLI
description: The Kryton CLI — @azrtydxb/kryton-init one-shot installer and @azrtydxb/kryton-mcp stdio shim.
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="note" title="Source of truth">
This page mirrors [`kryton/docs/CLI.md`](https://github.com/azrtydxb/kryton/blob/master/docs/CLI.md). If the two diverge, the source file in the repo wins. Future iterations may switch to a build-time `?raw` import; for now it's a curated copy.
</Aside>
...
Both packages publish independently to npm under `@azrtydxb/`. Tag the monorepo with `cli-v<semver>` (e.g. `cli-v0.1.0`) — the [`cli-publish`](https://github.com/azrtydxb/kryton/blob/master/.github/workflows/cli-publish.yml) workflow builds and tests both packages, then publishes them in order (`kryton-mcp` first so the shim is on the registry by the time `kryton-init` recommends it).

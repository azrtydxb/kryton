---
title: Changelog
description: Kryton release notes — Unreleased, then descending by version.
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="note" title="Source of truth">
This page mirrors [`kryton/CHANGELOG.md`](https://github.com/azrtydxb/kryton/blob/master/CHANGELOG.md), regenerated from Conventional Commits by `git-cliff`. If the two diverge, the source file in the repo wins. Future iterations may switch to a build-time `?raw` import; for now it's a curated copy.
</Aside>
...
- **`api.notes.*`, `api.storage.*`, `api.editor.*` client plugin namespaces** — first-class APIs for note CRUD, per-plugin KV storage, and editor cursor / selection / transactions; documented in [`docs/PLUGINS.md`](https://github.com/azrtydxb/kryton/blob/master/docs/PLUGINS.md).
...
- See [CLI](/kryton/advanced/reference/cli/) for usage.
...
- **WebSocket authentication** — session validation on upgrade handshake, disabled-user check, 64 KB message size limit.
...
- **Input validation** — Zod schemas on all endpoints (11 new schemas), content size limits (1 MB), path length limits (500 chars).
...
- **Zod validation everywhere** — all admin, shares, canvas, folder, and rename routes now validated.
...
- **D3 graph extracted** into `useD3Graph` hook with named config constants (was a 275-line useEffect with 12 magic numbers).
...
- **SPEC.md rewritten** — was describing a single-user TypeORM app, now matches actual v3 architecture.
